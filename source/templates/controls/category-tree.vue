			...
			<div>
				<div class="app-category-tree-node">
					...
					<span v-if="child.childs > 0" v-on:click.stop="drilldown(child)">
						<i class="fas" :class="[node != null ? 'fa-minus-circle' : 'fa-plus-circle']"></i>
						${child.name}
					</span>
					<span v-else v-on:mouseover="preselected = child" v-on:mouseleave="preselected = null" v-on:click.stop="select(child)">
						<i class="far" :class="(preselected === child || (branch != null && branch.value === child)) ? 'fa-dot-circle' : 'fa-circle'"></i>
						${child.name}
					</span>
				</div>
					...
					<table cellpadding="0" cellspacing="0">
						<tr>
							...
							<td :class="{'app-category-tree-subnode-content' : level != null && level >= 0}" ref="test">
								<category-tree :level="level != null ? level + 1 : 0" :reference="node.id" :branch="branch" v-on:select="onSelected"></category-tree>
							</td>
						</tr>
					</table>
				</div>
