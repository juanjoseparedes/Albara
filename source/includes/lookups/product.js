...
anxeb.vue.include.component('lookup-product', function (helpers) {
	return {
		template : '/lookups/product.vue',
		created  : function () {
			var _self = this;
					...
					if (_self.model.id) {
						if (_self.invalidate != null && _self.invalidate.length > 0) {
							if (_self.invalidate.includes(_self.model.id) === true) {
								_self.$root.log(helpers.tools.requiredError('model', ['id'], 'Producto previamente asociado')).exception();
							} else {
								modal.close();
						...
						_self.$root.log(helpers.tools.requiredError('model', ['id'], 'Debe seleccionar un producto')).exception();
					}
				}
			}, {
				text   : 'Cancelar',
