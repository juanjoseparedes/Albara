...
anxeb.vue.include.scope('composing/providers', function (helpers, instance) {
	return {
		mounted : function () {
			var _self = this;
			...
			helpers.api.get('/providers').then(function (res) {
				...
				if (_self.selectedProviderId != null) {
					...
					if (_self.selection.provider == null) {
						...
						helpers.root.navigate('/composing/providers');
					}
				}
		...
		watch   : {
			$route(to, from) {
				if (to.path === '/composing/providers') {
					this.reset();
				}
			},
			...
			updateHeader      : function () {
				...
				if (_self.selectedInstitutionId != null) {
					_self.page.menu.add({
						...
						action  : function () {
							helpers.root.navigate('/billing/providers/' + _self.$route.params.providerId + '/institutions/' + _self.selectedInstitutionId + '/detail?month=' + _self.selectedMonth);
						}
					});
				} else {
				...
				if (_self.selection.provider) {
					if (_self.loaded === false && _self.$route.params.providerId != null) {
						if (_self.selectedInstitutionId != null) {
							...
							helpers.root.navigate('/composing/providers/' + _self.$route.params.providerId + '/institutions/' + institution.id + '/detail');
						} else {
							helpers.root.navigate('/composing/providers/' + _self.selection.provider.id + '/institutions');
						}
					} else {
						_self.selection.institution = null;
						_self.selectedInstitutionId = null;
